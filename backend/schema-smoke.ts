...
type SmokeTx = Parameters<Parameters<Db['transaction']>[0]>[0];
type Probe = (label: string, fn: () => Promise<unknown>) => Promise<void>;

// Drizzle wraps the pg error: walk the cause chain for code 23505.
function violationText(error: unknown): string {
  ...
  return chain
    .map((e) =>
      e instanceof Error
        ? `${e.message} ${(e as { code?: unknown }).code ?? ''}`
        : String(e),
    )
    .join(' | ');
}

// Probes run inside a savepoint so a 25P02 abort never poisons the outer tx.
function makeProbe(tx: SmokeTx, ok: (label: string) => void): Probe {
  let probe = 0;
  return async (label, fn) => {
    ...
    try {
      ...
      await tx.execute(sql`ROLLBACK TO SAVEPOINT ${sp}`);
      const text = violationText(error);
...
async function seedOrgFixtures(
  ...
  await probe('faculty<->user 1-to-1', () =>
    tx
      .insert(schema.users)
  ...
  await probe('role_permission dedupe', () =>
  ...
  ok('insert employee');
  await probe('user<->employee 1-to-1', () =>
  ...
  await probe('usersDepartments dedupe', () =>
...
async function seedAcademicFixtures(
  ...
  ok('insert result');
  await probe('result per semester dedupe', () =>
    tx
      .insert(schema.results)
...
async function checkTimestampBump(tx: SmokeTx, ok: (label: string) => void) {
...
async function checkRelations(
  ...
  if (withRelations?.faculty?.id === facultyId && withRelations?.employee?.role?.name === 'smoke-role') {
...
const main = async () => {
  ...
  try {
    await db.transaction(async (tx) => {
      const probe = makeProbe(tx, ok);
      const { faculty, user } = await seedOrgFixtures(tx, probe, ok);
      await seedAcademicFixtures(tx, probe, ok, faculty.id);
      await checkTimestampBump(tx, ok);
      await checkRelations(tx, ok, user.id, faculty.id);
      throw new Rollback();
    });
  } catch (error) {
